...
function contactValidator() {
    ...
    window.setTimeout(function () {
        if ($('#mobile').length > 0) {
            telnumber = window.intlTelInput(document.querySelector("#mobile"), {
                utilsScript: "assets/intl-tel-input/js/utils.js?21",
            });
            telnumber.setCountry("in");
        }
    }, 3000);
    ...
    e < 1 || (e.validator(),
        e.on("submit", function (t) {
            ...
            if (!t.isDefaultPrevented()) {
                ...
                //     grecaptcha.execute('6LfTXJwbAAAAAJIpeIlqZ59ZkafKDeRYxGX8AQt2', { action: 'contact' }).then(function (token) {
                //         var recaptchaResponse = document.getElementById('recaptchaResponse');
                //         recaptchaResponse.value = token;
                //     });
                // });
                if ($('#mobile').length > 0) {
                    $('#country_code').val(telnumber.getSelectedCountryData().dialCode);
                }


                if($('#listing_id').length > 0){
                    let project_list = { "Vivaan Elysium": "Vivaan%20Elysium", "Vivaan Elementos": "Vivaan%20Elementos", "Vivaan Eminence": "Vivaan%20Eminence", "Vivaan Infinity": "Vivaan%20Infinity", "Vivaan Ixora": "Vivaan%20Ixora", "Vivaan Aura": "Vivaan%20Aura", "Vivaan Essence": "Vivaan%20Essence", "Vivaan Solitaire": "Vivaan%20Solitaire", "Vivaan Oliver": "Vivaan%20Oliver", "Vivaan Orbit": "Vivaan%20Orbit" };
                    ...
                    if (!(mobile.indexOf(country_code) == 0)) {
                    } else {
                    ...
                    $.ajax({
                        url: 'https://crm.vivaanrealty.com/daebuild/api_lead_website.php?authtoken=6gfG97fTGrx7ELcGXqAA5UGJXTHx94&action=insertLeads&project_name='+project_list[listing_id]+'&name='+encodeURI(name)+'&email='+encodeURI(email)+'&phone_no='+encodeURI(mobile)+'&remarks='+encodeURI(subject+' | '+message)+'&city=&source=Website&sub_source=&lead_date='+datetime,
                        type: "GET",
                ...
                return $.ajax({
                    type: "POST",
                    url: e.data('form-path') + '&recaptcha_response=' + $('#recaptchaResponse').val(),
                    // data: $(this).serialize(),
            ...
            // }else{
            //     var o = '<div class="alert alert-danger alert-dismissable">Invalid Email Id</div>';
            //       ($(".messages").html(o));
            //       setTimeout(function() {
            //           $(".messages").html("")
            //       }, 5000);
...
function contact1Validator() {
    var telnumber1;
    window.setTimeout(function () {
        if ($('#mobile1').length > 0) {
            telnumber1 = window.intlTelInput(document.querySelector("#mobile1"), {
                utilsScript: "assets/intl-tel-input/js/utils.js?21",
            });
            telnumber1.setCountry("in");
        }
    }, 3000);
    var e = $("#contact-form1");
    e < 1 || (e.validator(),
        e.on("submit", function (t) {
            ...
            if (!t.isDefaultPrevented()) {
                ...
                if ($('#mobile1').length > 0) {
                    $('#country_code1').val(telnumber1.getSelectedCountryData().dialCode);
                }
                ...
                return $.ajax({
                    ...
                    url: e.data('form-path') + '&recaptcha_response=' + $('#recaptchaResponse').val(),
                    // data: $(this).serialize(),
                    data: new FormData($("#contact-form1")[0]),
                    processData: false,
                    contentType: false,
                    success: function (t) {
                        if (t.status) {
...
function brochureValidator() {
    ...
    window.setTimeout(function () {
        if ($('#mobile2').length > 0) {
            telnumber2 = window.intlTelInput(document.querySelector("#mobile2"), {
                utilsScript: "assets/intl-tel-input/js/utils.js?21",
            });
            telnumber2.setCountry("in");
        }
    }, 3000);
    ...
    e < 1 || (e.validator(),
        e.on("submit", function (t) {
            ...
            if (!t.isDefaultPrevented()) {
                ...
                if ($('#mobile2').length > 0) {
                    $('#country_code2').val(telnumber2.getSelectedCountryData().dialCode);
                }

                
                
                if($('#listing_id2').length > 0){
                    let project_list = { "Vivaan Elysium": "Vivaan%20Elysium", "Vivaan Elementos": "Vivaan%20Elementos", "Vivaan Eminence": "Vivaan%20Eminence", "Vivaan Infinity": "Vivaan%20Infinity", "Vivaan Ixora": "Vivaan%20Ixora", "Vivaan Aura": "Vivaan%20Aura", "Vivaan Essence": "Vivaan%20Essence", "Vivaan Solitaire": "Vivaan%20Solitaire", "Vivaan Oliver": "Vivaan%20Oliver", "Vivaan Orbit": "Vivaan%20Orbit" };
                    ...
                    var listing_id = $('#listing_id2').val();
                ...
                return $.ajax({
                    ...
                    url: e.data('form-path') + '&recaptcha_response=' + $('#recaptchaResponse').val(),
                    // data: $(this).serialize(),
                    data: new FormData($("#brochure-form")[0]),
                    processData: false,
                    contentType: false,
                    success: function (t) {
                        if (t.status) {
                            ...
                            // var n = "alert-" + t.type
                            //   , i = t.message
                            //   , o = '<div class="alert ' + n + ' alert-dismissable">' + i + "</div>";
...
function initMap() {
    ...
    function t(n) {
        data_overlay(),
            ...
            }(), background(), initMap(), SliderProject(), slick_client(i),
            function (n) {
            ...
            }), e('[data-dsn-col="4"] .slick-slider').slick({
                // Disable preloading of all images
                preloadImages: false,
                // Enable lazy loading
                lazy: true,
            ...
            }().run(), e("a.vid").YouTubePopUp(), contactValidator(), contact1Validator(), brochureValidator()
    }
    ...
    function n() {
        ...
        void 0 !== o ? a = o : a.top <= 70 && (a = s), r.getListener(function (e) {
            ...
            // console.log(n);
            // console.log(a.top);
            void 0 !== a && (o = a.top - 300), n > o ? l < n ? t.addClass("nav-bg").addClass("hide-nave") : t.removeClass("hide-nave") : t.removeClass("nav-bg").removeClass("hide-nave"), l = n
        })
    }
    ...
    function a() {
        ...
        return {
            ...
            start: function () {
                ...
                }), e(".dsn-link-paginate").on("click", function (t) {
                    t.preventDefault();
                    let n = i;
                    r.isScroller(!0) && (n = r.getScrollbar()), TweenLite.to(n, 1.5, {
                        scrollTo: r.getScrollbar().offset.y + e($(this).data('href')).offset().top,
                        ease: Expo.easeInOut
                    })
        ...
        l = function () {
            ...
            return {
                ...
                changeColor: function () {
                    const n = "v-light";
                    var a = o.hasClass(n);
                    e('[data-dsn="color1"]').each(function () {
    ...
    }), contactValidator(), contact1Validator(), brochureValidator(), n()
}(jQuery);
...
$(document).ready(function () {
    ...
    var modal1 = $("#contactModal");
    var modal2 = $("#brochureModal");
    ...
    $('.day-night').on('click', function () {
        if ($('meta[name=mode]').attr("content") == 'night') {
            $('meta[name=mode]').attr("content", 'day');
            $('body').addClass('v-light');
            $('.modal-content').css('background', '#fff');
            $('.close').css('color', '#000');
        } else {
            $('meta[name=mode]').attr("content", 'night');
            $('.modal-content').css('background', '#101010');
